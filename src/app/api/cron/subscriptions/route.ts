...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    const expiringIn1Day = expiringSubscriptions.filter(
      (sub) => sub.endDate && sub.endDate <= oneDayFromNow && sub.endDate > now,
    );
    const expiringIn3Days = expiringSubscriptions.filter(
      (sub) =>
        sub.endDate &&
        sub.endDate <= threeDaysFromNow &&
        sub.endDate > oneDayFromNow,
    );
    const expiringIn7Days = expiringSubscriptions.filter(
      (sub) =>
        sub.endDate &&
    ...
    return NextResponse.json({ error: "کار Cron انجام نشد" }, { status: 500 });
  }
}
