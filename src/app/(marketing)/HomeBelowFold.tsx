...
export default function HomeBelowFold({ plans }: { plans: Plan[] }) {
  return (
    <>
      {/* Features Section */}
      <section id="features" className="py-24">
        <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <MagicBento
            cards={[
              { title: "تحلیل احساسات متن", description: "تشخیص دقیق احساسات پنهان در پیام‌های شما با استفاده از هوش مصنوعی", label: "هوش مصنوعی", icon: <MessageCircle className="w-5 h-5" /> },
              { title: "تحلیل احساسات صوت", description: "درک لحن و احساس صدا برای ارتباطی عمیق‌تر و انسانی‌تر", label: "صوتی", icon: <Activity className="w-5 h-5" /> },
              { title: "گفتگوی همدلانه", description: "پاسخ‌های طراحی شده بر اساس اصول روانشناسی برای حمایت عاطفی", label: "همدلی", icon: <Heart className="w-5 h-5" /> },
              { title: "مدیتیشن و آرام‌سازی", description: "ارائه تمرین‌های تنفسی و مدیتیشن متناسب با وضعیت فعلی شما", label: "آرامش", icon: <Wind className="w-5 h-5" /> },
              { title: "محتوای شخصی‌سازی شده", description: "پادکست‌ها و مقالات پیشنهادی بر اساس نیازهای روانی شما", label: "شخصی‌سازی", icon: <Brain className="w-5 h-5" /> },
              { title: "گزارش پیشرفت روانی", description: "نمودارها و تحلیل‌های دوره‌ای از تغییرات خلقی شما", label: "تحلیل", icon: <LineChart className="w-5 h-5" /> },
            ]}
      ...
      <section className="relative">
        ...
        <div className="relative">
          {[
            { step: 1, title: "احساست را بنویس", desc: "هر چه در دل داری بنویس. آراما با صبر و بدون قضاوت گوش می‌دهد. هیچ محدودیتی نیست.", icon: MessageCircle, bg: "from-primary/10 to-secondary/5", accent: "bg-primary/10" },
            { step: 2, title: "آراما تحلیل می‌کند", desc: "هوش مصنوعی احساسات پنهان و نیازهای واقعی تو را شناسایی می‌کند و الگوهای رفتاری را درک می‌کند.", icon: Brain, bg: "from-accent/10 to-primary/5", accent: "bg-accent/10" },
            { step: 3, title: "پاسخ همدلانه دریافت کن", desc: "پاسخ‌هایی بر اساس اصول روانشناسی، شخصی‌سازی شده برای تو. هر پاسخ مخصوص توست.", icon: Heart, bg: "from-secondary/10 to-accent/5", accent: "bg-secondary/10" },
            { step: 4, title: "تمرین مناسب دریافت کن", desc: "مدیتیشن، تنفس عمیق یا محتوای پیشنهادی متناسب با حالت فعلی‌ات. همه چیز در لحظه آماده است.", icon: Wind, bg: "from-primary/10 to-accent/5", accent: "bg-primary/10" },
            { step: 5, title: "پیشرفتت را مشاهده کن", desc: "نمودارها و تحلیل‌های دوره‌ای برای دیدن مسیر رشد و آرامش. قدم به قدم به جلو.", icon: LineChart, bg: "from-accent/10 to-secondary/5", accent: "bg-accent/10" },
          ].map((item, i) => {
            const textRight = i % 2 === 0;
            return (
              <div key={i} className="sticky top-20 h-[calc(100vh-5rem)] overflow-hidden" style={{ zIndex: i + 1 }}>
                <div className={`w-full h-full bg-linear-to-br ${item.bg} bg-card`}>
                  <div className="w-full h-full flex flex-col md:flex-row items-center">
                    <div className={`w-full md:w-1/2 flex flex-col justify-center px-8 sm:px-12 lg:px-20 py-12 md:py-0 ${textRight ? "md:order-2" : "md:order-1"}`}>
                      <motion.div
                      ...
                      >
                        <h3 className="text-3xl md:text-4xl lg:text-5xl font-black text-foreground mb-5 leading-tight">{item.title}</h3>
                        <p className="text-lg md:text-xl text-muted-foreground leading-relaxed max-w-lg">{item.desc}</p>
                      </motion.div>
                    </div>
                    <div className={`w-full md:w-1/2 flex items-center justify-center py-8 md:py-0 ${textRight ? "md:order-1" : "md:order-2"}`}>
                      <motion.div
                      ...
                      >
                        ...
                        <div className={`absolute -top-6 -right-6 w-20 h-20 rounded-full ${item.accent} blur-2xl`} />
                        <div className={`absolute -bottom-4 -left-4 w-16 h-16 rounded-full ${item.accent} blur-xl`} />
          ...
          <div className={`grid grid-cols-1 gap-8 ${plans.length === 1 ? 'md:grid-cols-1 max-w-md mx-auto' : plans.length === 2 ? 'md:grid-cols-2 max-w-2xl mx-auto' : plans.length === 3 ? 'md:grid-cols-3' : 'md:grid-cols-2 lg:grid-cols-4'}`}>
            {plans.length > 0 ? (
              plans.map((plan, index) => (
                <div key={plan.id} className={`relative bg-card border rounded-3xl p-8 shadow-sm flex h-full flex-col ${index === 1 ? "border-2 border-primary transform md:-translate-y-4 shadow-lg" : "border-border"}`}>
                  {index === 1 && (
                    <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 bg-primary text-primary-foreground px-4 py-1 rounded-full text-sm font-bold">محبوب‌ترین</div>
                  )}
                  ...
                  <div className="mb-8">
                    ...
                    <span className="text-muted-foreground mr-1">{plan.price === 0 ? "تومان" : `تومان / ${plan.durationDays === 30 ? "ماه" : plan.durationDays === 365 ? "سال" : ""}`}</span>
                  </div>
                  ...
                  <Button className={`w-full mt-auto ${index === 1 ? "bg-primary hover:bg-primary/90" : ""}`} variant={index === 1 ? "default" : "outline"} asChild>
                    <Link href="/dashboard">{plan.price === 0 ? "شروع رایگان" : "خرید اشتراک"}</Link>
                  </Button>
                </div>
              ))
            ) : (
              <div className="md:col-span-3 text-center text-muted-foreground">پلنی یافت نشد</div>
            )}
          </div>
        </div>
      </section>
      ...
      <section className="py-24 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 text-center">
          <motion.div initial={{ opacity: 0, y: 30 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="max-w-2xl mx-auto">
            ...
            <p className="text-xl text-muted-foreground mb-10">آراما همین الان آماده شنیدن حرف‌های توست.</p>
            <MagneticButton>
              <Button size="lg" className="h-16 px-10 text-lg bg-primary hover:bg-primary/90 rounded-full" asChild>
