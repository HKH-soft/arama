...
export default function MeditationPage() {
  ...
  useEffect(() => {
    const fetchTracks = async () => {
      try {
        const res = await fetch("/api/meditation-tracks");
  ...
  const selectMeditation = (track: MeditationTrack) => {
    if (!track) return;
      ...
      <div className="px-6 pb-6 space-y-6">
        {tracks.length > 0 && (
          <div className="bg-linear-to-r from-violet-500/20 via-purple-500/10 to-transparent rounded-xl p-6 border border-violet-500/10">
            ...
            <div className="flex items-center gap-4">
              <button
                onClick={() => selectMeditation(tracks[0])}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {loading ? (
            ...
            <p className="text-foreground/50">مدیتیشنی یافت نشد</p>
          )}
        </div>
      </div>
    </div>
