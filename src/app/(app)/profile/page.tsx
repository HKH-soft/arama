  ...
  useEffect(() => {
    const fetchPayments = async () => {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          const paymentsData = data.data && Array.isArray(data.data) ? data.data : Array.isArray(data) ? data : [];
  ...
  useEffect(() => {
    const fetchPlans = async () => {
      try {
        ...
        if (res.ok) {
          const data = await res.json();
          const plansData = data.data && Array.isArray(data.data) ? data.data : Array.isArray(data) ? data : [];
              ...
              <CardHeader className="text-center">
                ...
                {avatarPreview && (
                  <Button
                    variant="ghost"
                    size="sm"
                    className="mt-2 mx-auto text-destructive"
                ...
                <div className="space-y-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <span className="text-sm font-medium bg-green-100 text-green-800 px-2 py-1 rounded-full text-xs">
                      {userData?.subscription?.status === "ACTIVE" ? "فعال" : "رایگان"}
                    </span>
                  </div>
                </div>
              </CardContent>
              ...
              <CardHeader>
                <div className="flex items-center justify-between">
                  ...
                  <Badge variant={userData.subscription.status === "ACTIVE" ? "default" : "destructive"}>
                    {userData.subscription.status === "ACTIVE" ? "فعال" : "منقضی شده"}
                  </Badge>
                </div>
              </CardHeader>
              <CardContent>
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                  <div>
                    <h3 className="text-xl font-bold">{userData.subscription.plan.displayName}</h3>
                    <p className="text-muted-foreground">
                      {userData.subscription.plan.price === 0 ? "پلن رایگان" : `اشتراک ${userData.subscription.plan.durationDays} روزه`}
                    </p>
                    <div className="mt-4 space-y-2">
                      <div className="flex items-center gap-2">
                        <Calendar className="w-4 h-4 text-muted-foreground" />
                        <span>شروع: {userData.subscription.startDate ? new Date(userData.subscription.startDate).toLocaleDateString('fa-IR') : "-"}</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <Calendar className="w-4 h-4 text-muted-foreground" />
                        <span>پایان: {userData.subscription.endDate ? new Date(userData.subscription.endDate).toLocaleDateString('fa-IR') : "-"}</span>
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
                    <div className="text-2xl font-bold">
                      {userData.subscription.plan.price === 0 ? "رایگان" : `${userData.subscription.plan.price.toLocaleString()} تومان`}
                    </div>
                  </div>
                </div>
                ...
                <div>
                  ...
                  <ul className="grid grid-cols-1 md:grid-cols-2 gap-2">
                    {userData.subscription.plan.features.map((feature: string, idx: number) => (
                    ...
                    >
                      ...
                      <CardContent className="text-center flex-1 flex flex-col">
                        ...
                        <Button
                          ...
                          onClick={() => (window.location.href = "/api/payments/create")}
                          disabled={userData?.subscription?.plan.displayName === plan.displayName}
                        >
                          {userData?.subscription?.plan.displayName === plan.displayName ? "فعال" : "انتخاب پلن"}
                        </Button>
                      </CardContent>
                    </Card>
                  ))}
        ...
        <TabsContent value="billing" className="space-y-6 mt-6">
                        ...
                        <TableRow key={payment.id}>
                          ...
                          <TableCell>
                            <div className="flex items-center">
                              ...
                              {payment.amount.toLocaleString()} {payment.currency || "تومان"}
                            </div>
                          </TableCell>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {paymentsLoading ? <Skeleton className="h-6 w-16" /> : payments.filter(p => p.status === "SUCCESS").length}
                </div>
            ...
            <Card>
              ...
              <CardContent>
                <div className="text-2xl font-bold">
                  {paymentsLoading ? <Skeleton className="h-6 w-16" /> : payments.filter(p => p.status === "FAILED").length}
                </div>
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              <div className="flex items-center justify-between">
                ...
                <a href="/session-management#password">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className="font-medium">مدیریت نشست‌ها</h3>
                  <p className="text-sm text-muted-foreground">دستگاه‌های وارد شده را مدیریت کنید</p>
                </div>
                <a href="/session-management#sessions">
                  <Button variant="outline">
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">حذف دائمی حساب کاربری</p>
                </div>
